...
namespace App\Http\Controllers\Reports_Generators\Students\Diagnosticos;
...
class diagnosticos extends PDF
{
	...
	public function exportpdf($cod_diagnostico) {
	global $n;
 	$n=$cod_diagnostico;
   	$institucion=institucion::first();
   	$ano_act=$institucion->ANO_ACTIVO;
   	$diagnosticos=diagnostico::where('CODIGO_ESTUDIANTE','=',auth('student')->user()->CODIGO)->where('ANO','=',$ano_act)->where('COD_DIAGNOSTICO','=',$n)->get();
	...
	$positiony_current=$pdf::GetY();
 	...
 	$comm=visualizador_diagnostico::where('ESTUDIANTE','=',auth('student')->user()->CODIGO)->where('COD_DIAG','=',$cod_diagnostico)->first();
				$comentario_dircurso="";
				$comentario_padre="";
			...
			if(isset($comm)){
				...
				$nuevo=new visualizador_diagnostico();
				$nuevo->ESTUDIANTE=auth('student')->user()->CODIGO;
				$nuevo->COD_DIAG=$cod_diagnostico;
				$nuevo->VISTO="No";
				$nuevo->save();
				$comm=visualizador_diagnostico::where('ESTUDIANTE','=',auth('student')->user()->CODIGO)->where('COD_DIAG','=',$cod_diagnostico)->first();
			}
    ...
    $updateDetails = [ 'VISTO' => "Si","FECHA_VISTO"=>$datetime]; 
    $calificacion2=visualizador_diagnostico::where('ESTUDIANTE','=',auth('student')->user()->CODIGO)->where('COD_DIAG','=',$cod_diagnostico)->update($updateDetails); 
    }
 	$pdf::SetFillColor(0,142,213);
 	$pdf::SetTextColor(255,255,255);
 	$pdf::SetFont('Arial','B',9);
