...
//Computes the number of lines a MultiCell of width w will take
    ...
    $this->cell(304,6,utf8_decode("$institucion->NOMBRE_ESTABLECIMIENTO"),1,"","C");
 	...
 	$this->cell(25,6,utf8_decode("PERIODO: I"),0,"","L");
...
class boletin_prueba extends PDF
{
	...
	public function exportpdf($curso_id) {
 	...
 	$consultasumanot=calificacion::where('CODIGO_ESTUDIANTE',$estudiante->CODIGO)->where('ANO_ACT',$institucion->ANO_ACTIVO)->sum('P1');
 	...
 	foreach($areas as $area){
 		...
 		$asignaturas=asignatura::where('id_area',$area->id)->where('id_curso',$curso_id)->get();
 			foreach($asignaturas as $asignatura){
 		        ...
 		        $logro_hacerx="";
 		        $logro_serx="";
 				$logros_saber=logro::where('id_asignatura',$asignatura->id)->where('PERIODO','1')->where('TIPO_LOGRO','SABER')->get();
 				$logros_hacer=logro::where('id_asignatura',$asignatura->id)->where('PERIODO','1')->where('TIPO_LOGRO','HACER')->get();
 				$logros_ser=logro::where('id_asignatura',$asignatura->id)->where('PERIODO','1')->where('TIPO_LOGRO','SER')->get();
				...
				$pdf::Cell(15, 4,"$periodo1", 1, 0, 'C', true);
				$promedio+=$periodo1;
				$pdf::SetFillColor(255,255,255);
				$pdf::SetTextColor(0,0,0);
				$pdf::SetFont('Arial','',7);
				$pdf::SetX(10);
				$pdf::Row(array("","$logro_saberx","$logro_hacerx","$logro_serx","","","","","","   $fallasp1","   $asignatura->IHS"));
