...
namespace App\Http\Controllers\Reports_Generators\Students\Boletines;
...
//Computes the number of lines a MultiCell of width w will take
    ...
    $this->cell(304,6,utf8_decode("$institucion->NOMBRE_ESTABLECIMIENTO"),1,"","C");
 	...
 	$this->cell(25,6,utf8_decode("PERIODO: III"),0,"","L");
 	$this->cell(55,6,utf8_decode("PROMEDIO DEL PERIODO: $promediototalperiodo1"),0,"","L");
 	$this->cell(25,6,utf8_decode("PUESTO: $puesto / $estudiantestotal"),0,"","L");
    ...
    return number_format($resultado, $digitos);
 	...
 	public function darpuesto($code,$curso_id){
 		...
 		$consultasumanot=calificacion::where('CODIGO_ESTUDIANTE',$estudiante->CODIGO)->where('ANO_ACT',$institucion->ANO_ACTIVO)->sum('P3');
...
class p3 extends PDF
{
	

	public function exportpdf($estudiante_cod) {
   	$institucion=institucion::first();
   	$h=historial_periodo::where('periodo_anual',$institucion->ANO_ACTIVO)->first();
 	$pdf=new PDF('L','mm','Legal');
 	$pdf::SetAutoPageBreak(true,2); 
 	
 	$estudiante=estudiante::where('CODIGO',$estudiante_cod)->where('ESTADO_DEL_ESTUDIANTE','ACTIVO')->first();
 	...
 	foreach($areas as $area){
 		...
 		$asignaturas=asignatura::where('id_area',$area->id)->where('id_curso',$curso_id)->get();
 			foreach($asignaturas as $asignatura){
 		        ...
 		        $logro_serx="";
 		        $defc=0;
 				$logros_saber=logro::where('id_asignatura',$asignatura->id)->where('PERIODO','3')->where('TIPO_LOGRO','SABER')->get();
 				$logros_hacer=logro::where('id_asignatura',$asignatura->id)->where('PERIODO','3')->where('TIPO_LOGRO','HACER')->get();
 				$logros_ser=logro::where('id_asignatura',$asignatura->id)->where('PERIODO','3')->where('TIPO_LOGRO','SER')->get();
				...
				$positiony_current=$pdf::GetY();
 				...
 				$defc=($periodo1+$periodo2+$periodo3)/3;
				...
				$pdf::Cell(15, 4,"$periodo3", 1, 0, 'C', true);
				...
				$pdf::SetX(10);
				$pdf::Row(array("","$logro_saberx","$logro_hacerx","$logro_serx","","","","","","   $fallasp3","   $asignatura->IHS"));
